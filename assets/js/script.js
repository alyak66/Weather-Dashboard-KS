    ...
    fetch(forecastUrl).then(function(response) {
        if (response.ok) {
            response.json().then(function(data) {
                ...
                //get today and format it so it can be easily compared with the dates returned by the api call I want to ignore any data with a date that matches today.
                var today = moment().format("YYYY-MM-DD");
                //console.log(today);
                for (var i=0; i<data.list.length; i++){
    
                    //OpenWeather returns a value called dt_txt which is the date and the time separated by a " ". I split this string and save it to dateTime where [0] is the date and [1] is the time.
...
var formSubmitHandler = function(event) {
    ...
    if (searchCity) {
        ...
        //if getting the city name would return anything false
        return;
    }
}
...
//clears the data in the results column
